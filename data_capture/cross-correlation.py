...
def save_cross_corr_plot(fig, csi_file, corr_type):
    ...
    if os.path.exists(plot_file):
        print(f"Plot {plot_file} already exists. Skipping.")
        return
...
for csi_file, audio_file in zip(csi_files_amplitude, audio_files):
    try:
        ...
        # Check if both CSV and plot already exist
        base_name = os.path.splitext(csi_file)[0]
        csv_output = f'data_capture/cross_correlation_amplitude/{base_name}_cross_corr_amplitude.csv'
        plot_output = f'data_capture/cross_correlation_amplitude_plot/{base_name}_cross_corr_amplitude_plot.png'
        
        if os.path.exists(csv_output) and os.path.exists(plot_output):
            print(f"Cross-correlation and plot for {csi_file} already exist. Skipping.")
            continue
...
for csi_file, audio_file in zip(csi_files_phase, audio_files):
    try:
        ...
        # Check if both CSV and plot already exist
        base_name = os.path.splitext(csi_file)[0]
        csv_output = f'data_capture/cross_correlation_phase/{base_name}_cross_corr_phase.csv'
        plot_output = f'data_capture/cross_correlation_phase_plot/{base_name}_cross_corr_phase_plot.png'
