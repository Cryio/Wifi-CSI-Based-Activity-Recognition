...
def wav_to_matrix_librosa(wav_file, target_sr, method="polynomial"):
    """
...
def choose_resampling_method_with_timeout():
    """
    Prompt the user to select a resampling method with a 5-second timeout. If no input is received, the polynomial method is selected.
    ...
    print("You have 5 seconds to choose, or 'Polynomial interpolation' will be selected by default.")

    def input_with_timeout():
        # return input("Enter the number corresponding to your choice (1-7): ")
        return 3
    choice = [None]  # Store the user's choice in a list to make it mutable from within the thread

    def get_choice():
        try:
            choice[0] = input_with_timeout()
        except EOFError:
            pass
    ...
    # Wait for 5 seconds, then check if the user has provided input
    input_thread.join(5)

    # If the user has not made a choice in time, default to 'polynomial'
    if not choice[0]:
        print("No input received. Defaulting to 'Polynomial interpolation'.")
        return "polynomial"
    ...
    return methods.get(choice[0], "polynomial")  # Default to polynomial if invalid choice
...
def convert_all_wavs_to_csv(audio_dir, output_dir, csi_dir, resampling_method):
    ...
    for filename in os.listdir(audio_dir):
        if filename.endswith(".wav"):
            ...
            # Skip if the CSI sampling rate is NaN or zero
            if not np.isfinite(csi_sampling_rate) or csi_sampling_rate <= 0:
                print(f"Invalid CSI sampling rate for {filename}, skipping.")
                continue
            ...
            print(f"Saved resampled audio matrix to {output_csv_path}")
...
if __name__ == "__main__":
    ...
    # User chooses the resampling method, with a timeout defaulting to polynomial
    resampling_method = choose_resampling_method_with_timeout()
